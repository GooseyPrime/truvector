...
export default function App() {
  return (
    <div className="min-h-screen text-slate-200 p-4 md:p-8 font-sans selection:bg-emerald-500/30 relative overflow-hidden">
...
const PARTICLE_COUNT_DESKTOP = 200; // Increased from 100
const PARTICLE_COUNT_MOBILE = 100;  // Increased from 60
const CONNECTION_RADIUS = 190;      // Increased from 160 to draw more lines
...
function KinematicBackground() {
  ...
  return (
    <canvas
      ref={canvasRef}
      className="absolute inset-0 w-full h-full pointer-events-auto bg-[#020617]"
      style={{ zIndex: 0 }}
      aria-hidden
    />
  );
